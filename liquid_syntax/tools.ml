...
let remove_prefix text prefix =
  if starts_with text prefix then
    String.sub text ~pos:(String.length prefix)
      ~len:(String.length text - String.length prefix)
  else text
...
let remove_list_prefix lst prefix =
  List.sub lst ~pos:(List.length prefix)
    ~len:(List.length lst - List.length prefix)

type ('acc, 'curr) unfold_notifier = Next of 'acc * 'curr | Stop of 'acc
...
type ('acc, 'curr) fold_until_notifier = Forward of 'acc | Done of 'acc
...
let contains lst item = List.mem lst item ~equal:( = )
