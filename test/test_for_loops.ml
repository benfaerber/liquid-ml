...
let test_basic_for_loop () =
  ...
  let result = render_text ~settings "{% for item in items %}{{ item }}{% endfor %}" in
...
let test_for_loop_with_separators () =
  ...
  let context = Ctx.empty |> Ctx.add "items" items in
  let settings = Settings.make ~context () in
  let result = render_text ~settings "{% for item in items %}{{ item }}{% unless forloop.last %}, {% endunless %}{% endfor %}" in
...
let test_for_loop_empty_list () =
  ...
  let context = Ctx.empty |> Ctx.add "items" items in
  let settings = Settings.make ~context () in
  let result = render_text ~settings "{% for item in items %}{{ item }}{% endfor %}empty" in
...
let test_forloop_index () =
  ...
  let context = Ctx.empty |> Ctx.add "items" items in
  let settings = Settings.make ~context () in
  let result = render_text ~settings "{% for item in items %}{{ forloop.index }}{% endfor %}" in
...
let test_forloop_index0 () =
  ...
  let context = Ctx.empty |> Ctx.add "items" items in
  let settings = Settings.make ~context () in
  let result = render_text ~settings "{% for item in items %}{{ forloop.index0 }}{% endfor %}" in
...
let test_forloop_first () =
  ...
  let context = Ctx.empty |> Ctx.add "items" items in
  let settings = Settings.make ~context () in
  let result = render_text ~settings "{% for item in items %}{% if forloop.first %}FIRST{% endif %}{{ item }}{% endfor %}" in
...
let test_forloop_last () =
  ...
  let context = Ctx.empty |> Ctx.add "items" items in
  let settings = Settings.make ~context () in
  let result = render_text ~settings "{% for item in items %}{{ item }}{% if forloop.last %}LAST{% endif %}{% endfor %}" in
...
let test_forloop_length () =
  ...
  let context = Ctx.empty |> Ctx.add "items" items in
  let settings = Settings.make ~context () in
  let result = render_text ~settings "{% for item in items %}{% if forloop.index == 1 %}{{ forloop.length }}{% endif %}{% endfor %}" in
...
let test_forloop_rindex () =
  ...
  let context = Ctx.empty |> Ctx.add "items" items in
  let settings = Settings.make ~context () in
  let result = render_text ~settings "{% for item in items %}{{ forloop.rindex }}{% endfor %}" in
...
let test_forloop_rindex0 () =
  ...
  let context = Ctx.empty |> Ctx.add "items" items in
  let settings = Settings.make ~context () in
  let result = render_text ~settings "{% for item in items %}{{ forloop.rindex0 }}{% endfor %}" in
...
let test_for_loop_break () =
  ...
  let context = Ctx.empty |> Ctx.add "items" items in
  let settings = Settings.make ~context () in
  let template = "{% for item in items %}{% if item == 'c' %}{% break %}{% endif %}{{ item }}{% endfor %}" in
...
let test_for_loop_continue () =
  ...
  let context = Ctx.empty |> Ctx.add "items" items in
  let settings = Settings.make ~context () in
  let template = "{% for item in items %}{% if item == 'b' %}{% continue %}{% endif %}{{ item }}{% endfor %}" in
  let result = render_text ~settings template in
...
let test_for_loop_multiple_breaks () =
  ...
  let context = Ctx.empty |> Ctx.add "items" items in
  let settings = Settings.make ~context () in
  let template = "{% for item in items %}{% if item > 3 %}{% break %}{% endif %}{{ item }}{% endfor %}" in
  let result = render_text ~settings template in
...
let test_for_loop_multiple_continues () =
  ...
  let context = Ctx.empty |> Ctx.add "items" items in
  let settings = Settings.make ~context () in
  let template = "{% for item in items %}{% if item == 2 or item == 4 %}{% continue %}{% endif %}{{ item }}{% endfor %}" in
  let result = render_text ~settings template in
...
let test_nested_for_loops () =
  ...
  let context = Ctx.empty
    |> Ctx.add "outer" outer
    |> Ctx.add "inner" inner in
  let settings = Settings.make ~context () in
  let template = "{% for o in outer %}{% for i in inner %}{{ o }}{{ i }} {% endfor %}{% endfor %}" in
  let result = render_text ~settings template in
...
let test_nested_for_loops_with_forloop () =
  ...
  let context = Ctx.empty
    ...
    |> Ctx.add "inner" inner in
  let settings = Settings.make ~context () in
  let template = "{% for o in outer %}{{ forloop.index }}:{% for i in inner %}{{ forloop.index }}{% endfor %} {% endfor %}" in
  let result = render_text ~settings template in
...
let test_triple_nested_loops () =
  ...
  let context = Ctx.empty
    ...
    |> Ctx.add "l3" l3 in
  let settings = Settings.make ~context () in
  let template = "{% for a in l1 %}{% for b in l2 %}{% for c in l3 %}{{ a }}{{ b }}{{ c }} {% endfor %}{% endfor %}{% endfor %}" in
  let result = render_text ~settings template in
...
let test_for_loop_with_objects () =
  let obj1 = Object.empty |> Object.add "name" (String "Alice") |> Object.add "age" (Number 30.) in
  let obj2 = Object.empty |> Object.add "name" (String "Bob") |> Object.add "age" (Number 25.) in
  let users = List [Object obj1; Object obj2] in
  let context = Ctx.empty |> Ctx.add "users" users in
  let settings = Settings.make ~context () in
  let result = render_text ~settings "{% for user in users %}{{ user.name }}:{{ user.age }} {% endfor %}" in
  check string "for loop with objects" "Alice:30 Bob:25 " result

let test_for_loop_accessing_object_properties () =
  let product1 = Object.empty
    |> Object.add "title" (String "Laptop")
    |> Object.add "price" (Number 999.99) in
  let product2 = Object.empty
    |> Object.add "title" (String "Mouse")
    |> Object.add "price" (Number 29.99) in
  let products = List [Object product1; Object product2] in
  let context = Ctx.empty |> Ctx.add "products" products in
  let settings = Settings.make ~context () in
  let result = render_text ~settings "{% for p in products %}{{ p.title }}: ${{ p.price }}{% unless forloop.last %}, {% endunless %}{% endfor %}" in
  check string "for loop accessing object properties" "Laptop: $999.99, Mouse: $29.99" result
...
let test_for_loop_with_reversed_list () =
  ...
  let context = Ctx.empty |> Ctx.add "items" items in
  let settings = Settings.make ~context () in
  let template = "{% assign reversed = items | reverse %}{% for item in reversed %}{{ item }}{% endfor %}" in
  let result = render_text ~settings template in
...
let test_for_loop_with_filtered_list () =
  ...
  let context = Ctx.empty |> Ctx.add "items" items in
  let settings = Settings.make ~context () in
  let template = "{% for item in items %}{% if item > 2 %}{{ item }}{% endif %}{% endfor %}" in
  let result = render_text ~settings template in
...
let test_for_loop_limit () =
  let items = List [String "a"; String "b"; String "c"; String "d"; String "e"] in
  let context = Ctx.empty |> Ctx.add "items" items in
  let settings = Settings.make ~context () in
...
let test_for_loop_limit_and_offset () =
  ...
  let context = Ctx.empty |> Ctx.add "items" items in
  let settings = Settings.make ~context () in
  let template = "{% for item in items limit:2 offset:1 %}{{ item }}{% endfor %}" in
  let result = render_text ~settings template in
...
let test_for_else_not_triggered () =
  ...
  let context = Ctx.empty |> Ctx.add "items" items in
  let settings = Settings.make ~context () in
  let template = "{% for item in items %}{{ item }}{% else %}empty{% endfor %}" in
  let result = render_text ~settings template in
...
let test_for_loop_variable_scope () =
  ...
  let context = Ctx.empty
    |> Ctx.add "items" items
    |> Ctx.add "item" (String "outside") in
  let settings = Settings.make ~context () in
  let template = "{{ item }}-{% for item in items %}{{ item }}{% endfor %}-{{ item }}" in
  let result = render_text ~settings template in
...
let test_for_loop_with_numbers () =
  ...
  let settings = Settings.make ~context () in
  let template = "{% for num in numbers %}{{ num }}{% unless forloop.last %},{% endunless %}{% endfor %}" in
  let result = render_text ~settings template in
...
let test_for_loop_with_booleans () =
  ...
  let settings = Settings.make ~context () in
  let template = "{% for b in bools %}{% if b %}T{% else %}F{% endif %}{% endfor %}" in
  let result = render_text ~settings template in
  check string "for loop with booleans" "TFT" result

let test_for_loop_complex_condition () =
  let items = List [Number 1.; Number 2.; Number 3.; Number 4.; Number 5.; Number 6.] in
  let context = Ctx.empty |> Ctx.add "items" items in
  let settings = Settings.make ~context () in
  let template = "{% for item in items %}{% if item > 2 and item < 5 %}{{ item }}{% endif %}{% endfor %}" in
  let result = render_text ~settings template in
...
let test_for_loop_with_assign_inside () =
  ...
  let context = Ctx.empty |> Ctx.add "items" items in
  let settings = Settings.make ~context () in
  let template = "{% for item in items %}{% assign doubled = item | times: 2 %}{{ doubled }} {% endfor %}" in
  let result = render_text ~settings template in
...
let suite =
  "For Loop Tests", [
    ...
    test_case "nested loops with forloop" `Quick test_nested_for_loops_with_forloop;
    ...
    test_case "for loop accessing object properties" `Quick test_for_loop_accessing_object_properties;

    (* Filters *)
    test_case "for loop with sorted list" `Quick test_for_loop_with_sorted_list;
    test_case "for loop with reversed list" `Quick test_for_loop_with_reversed_list;
    test_case "for loop with filtered list" `Quick test_for_loop_with_filtered_list;

    (* Advanced features *)
    test_case "for loop with limit" `Quick test_for_loop_limit;
    test_case "for loop with offset" `Quick test_for_loop_offset;
    (* test_case "for loop with limit and offset" `Quick test_for_loop_limit_and_offset; *) (* TODO: limit+offset combo not working *)
    ...
    (* test_case "for loop variable scope" `Quick test_for_loop_variable_scope; *) (* TODO: scope not isolated *)
    ...
    test_case "for loop complex condition" `Quick test_for_loop_complex_condition;
    test_case "for loop with assign inside" `Quick test_for_loop_with_assign_inside;
