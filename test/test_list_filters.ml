...
let test_concat () =
  ...
  in
  let settings = Settings.make ~context () in
  let result = render_text ~settings "{{ list1 | concat: list2 | join: ',' }}" in
...
let test_map () =
  ...
  let context = Ctx.empty |> Ctx.add "users" (List [Object obj1; Object obj2]) in
  let settings = Settings.make ~context () in
  let result = render_text ~settings "{{ users | map: 'name' | join: ', ' }}" in
  check string "map filter" "Alice, Bob" result

let test_slice_list () =
  let context = Ctx.empty |> Ctx.add "items" (List [String "a"; String "b"; String "c"; String "d"]) in
  let settings = Settings.make ~context () in
...
let test_uniq () =
  let context = Ctx.empty |> Ctx.add "items" (List [String "a"; String "b"; String "a"; String "c"; String "b"]) in
  let settings = Settings.make ~context () in
  let result = render_text ~settings "{{ items | uniq | join: ',' }}" in
  check string "uniq filter" "a,b,c" result

let test_where_with_value () =
  let obj1 = Object.empty |> Object.add "name" (String "Alice") |> Object.add "active" (Bool true) in
  let obj2 = Object.empty |> Object.add "name" (String "Bob") |> Object.add "active" (Bool false) in
  let obj3 = Object.empty |> Object.add "name" (String "Charlie") |> Object.add "active" (Bool true) in
  let context = Ctx.empty |> Ctx.add "users" (List [Object obj1; Object obj2; Object obj3]) in
  let settings = Settings.make ~context () in
  let result = render_text ~settings "{% assign active_users = users | where: 'active', true %}{{ active_users | map: 'name' | join: ', ' }}" in
...
let test_where_truthy () =
  ...
  let context = Ctx.empty |> Ctx.add "users" (List [Object obj1; Object obj2; Object obj3]) in
  let settings = Settings.make ~context () in
  let result = render_text ~settings "{% assign active_users = users | where: 'active' %}{{ active_users | map: 'name' | join: ', ' }}" in
  check string "where filter truthy" "Alice, Charlie" result

let test_sort_natural () =
  let context = Ctx.empty |> Ctx.add "items" (List [String "item 10"; String "item 2"; String "item 1"]) in
  let settings = Settings.make ~context () in
...
let test_sort_natural_objects () =
  ...
  let context = Ctx.empty |> Ctx.add "items" (List [Object obj1; Object obj2; Object obj3]) in
  let settings = Settings.make ~context () in
  let result = render_text ~settings "{{ items | sort_natural: 'name' | map: 'name' | join: ',' }}" in
