...
let encode_decode_url reps encode_decode url =
  ...
  let ordered_reps =
    match encode_decode with Encode -> reps | Decode -> List.rev reps
  in
...
let encode_decode_text reps encode_decode text =
  let folder acc (unicode_value, number_encoded, name_encoded) =
    ...
    | Encode ->
        String.substr_replace_all acc ~pattern:unicode_value
          ~with_:number_encoded
    | Decode ->
        acc
        |> String.substr_replace_all ~pattern:number_encoded
             ~with_:unicode_value
...
let escape_param_url url =
  encode_url url |> String.substr_replace_all ~pattern:"+" ~with_:"%20"
