...
let apply_op op _ = function
  ...
  | Number a :: Number b :: _ -> Number (op a b) |> ok
  | other -> errc "operator accepts 2 numbers" other

let extract_key_from_object_list lst key =
  let lookup_key = function
    | Object obj -> (
        match Object.find_opt key obj with Some x -> x | _ -> Nil)
    | _ -> Nil
  in

  List.map lst ~f:lookup_key

type weight_units = Grams | Kilograms | Pounds | Ounces [@@deriving show]
...
let format_thousands_int n =
  ...
  aux r |> List.rev |> String.concat ~sep:","
...
let format_money_number n =
  let pieces =
    ...
    |> Float.to_string |> String.split ~on:'.'
  in
  ...
  let clean_sd =
    match String.length sd with 0 -> "00" | 1 -> sd ^ "0" | _ -> sd
  in
  fs ^ "." ^ clean_sd
