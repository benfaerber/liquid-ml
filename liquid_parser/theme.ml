...
let parse_render = function
  | LexValue (LexString filename) :: tokens -> (
      match tokens with
      | EOS :: tl -> Some (Render (filename, Ctx.empty, None), tl)
      | LexFor :: LexValue list :: LexAs :: LexValue (LexId id) :: EOS :: tl ->
          ...
          let iter =
            For (join id, iter_list, for_params_default, Block [ render ], None)
          in
          Some (iter, tl)
      | LexWith :: LexValue v :: EOS :: _ ->
          let render_ctx =
            Ctx.empty |> Ctx.add filename (lex_value_to_value v)
          in
...
let parse_form block_parser all_tokens =
  ...
  | LexValue (LexString filename) :: tokens -> (
      ...
      match tokens with
      | EOS :: _ -> Some (Render (formname, Ctx.empty, pbody), rest)
      ...
      | Comma :: tl -> (
          match parse_variable_context tl with
          | Some (ctx, _) -> Some (Render (formname, ctx, pbody), rest)
          | _ -> None)
      | _ -> None)
  | _ -> None
...
let parse_style block_parser = function
  | EOS :: tl ->
      ...
      let render =
        Render (Settings.style_tag, Ctx.empty, Some (block_parser body))
      in
      Some (render, rest)
  | _ -> None
