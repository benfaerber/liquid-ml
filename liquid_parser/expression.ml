...
let expression_from_tokens full_tokens =
  let rec aux tokens =
    let prefix, tail =
      ...
      | _ -> (Value (String Settings.skip), tokens)
    in

    let make_func id params tl =
      [ Func (List.hd_exn id, [ prefix ] @ to_exp_values params) ] @ aux tl
    in

    match tail with
    | Pipe
      :: LexValue (LexId [ "default" ])
      :: Colon
      :: LexValue p1
      :: Comma
      :: LexValue (LexId [ "allow_false" ])
      :: Colon
      :: LexValue p2
      :: tl ->
    ...
    | Pipe :: LexValue (LexId id) :: Colon :: LexValue p1 :: tl ->
        make_func id [ p1 ] tl
    | Pipe :: LexValue (LexId id) :: tl -> make_func id [] tl
    | _ -> []
  in
  ...
  | _ ->
      ...
      let rec unfold_into_func = function
        | Func (id, Value (String s) :: other_params) :: tl
          when s = Settings.skip ->
...
let parse_assignment _ tokens =
  let add_increment id modifier =
    ...
    Assignment (inc_id, Value (String modifier))
  in
  ...
  | Increment :: LexValue (LexId id) :: tl -> Some (add_increment id "plus", tl)
  | Decrement :: LexValue (LexId id) :: tl -> Some (add_increment id "minus", tl)
  | _ -> None
