...
let lex_number text =
  ...
  let neg_literal, t_text =
    if starts_with text "-" then ("-", remove_prefix text "-") else ("", text)
  in
  ...
  | first_group ->
      ...
      if starts_with decimal_part "." then
        ...
        | second_group ->
            to_num
              (t_first_group ^ "." ^ second_group)
              (remove_prefix second_group_part second_group)
      else to_num t_first_group decimal_part
...
let lex_delimited_string delim escaped_delim text =
  if starts_with text delim then
    ...
    Some
      (LexValue (LexString string_literal), remove_prefix text complete_literal)
  else None
...
let rec first_successful text = function
  | lexer :: other_lexers -> (
...
let lex_token text =
  let lexers =
    [ lex_keyword; lex_range; lex_bool; lex_string; lex_number; lex_id ]
  in
...
let lex_line_tokens text =
  ...
  let folder acc index =
    let chunk =
      String.sub t_text ~pos:index ~len:(String.length t_text - index)
    in
