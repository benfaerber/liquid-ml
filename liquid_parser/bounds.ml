...
let remove_last lst =
  match List.rev lst with _ :: hds -> List.rev hds | [] -> []
...
let has_conflict conflict_type bounds token =
  ...
  | Some conflicts ->
      let extract_tag b =
        match conflict_type with Start -> b.start | End -> b.stop
      in
...
let find_bounds tokens start_point =
  ...
  let bounds =
    ...
    | Some b -> b
    | None -> raise (Failure "This is not an opening tag")
  in
  ...
  let folder (tally, found) index =
    ...
    | _ when token = bounds.stop || has_end_conflict bounds token -> (
        ...
        let nf =
          if List.hd_exn found = List.last_exn tally then found @ [ index ]
          else found
        in
        let acc = (nt, nf) in
        match nt with [] -> Stop acc | _ -> Next (acc, index + 1))
    | _ when contains other_bounds token ->
        let nf =
          if first found = List.last_exn tally then found @ [ index ] else found
        in
        let acc = (tally, nf) in
        Next (acc, index + 1)
    | _ ->
  ...
  let _, found =
    unfold ([ start_point ], [ start_point ]) (start_point + 1) folder
  in
