...
let parse_forlike for_type block_parser tokens =
  let kw, is_tablerow =
    if for_type = ForLoop then (Tokens.LexFor, false) else (TableRow, true)
  in
  let do_for lid lex_val tl ~params =
    ...
    | 2 ->
        ...
        Some (for_loop, rest)
    | _ ->
        raise (Failure "A for loop can only have a body and an else statement")
  in

  match tokens with
  | st :: LexValue (LexId id) :: In :: LexValue lex_val :: EOS :: tl
    when st = kw ->
