...
let parse_when_statement block_parser case_id = function
  | When :: tl ->
      ...
      let rec unfold_into_or acc = function
        | a :: b :: tl ->
            unfold_into_or (Combine (Or, Combine (Or, a, b), acc)) tl
